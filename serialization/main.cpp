...
#include <iostream>
#include <iterator>
// #include <libs/serialization/example/simple_log_archive.hpp>
#include <sstream>
...
void saveLoad(const std::string& name, const GpsPosition& gps)
{
...
int main(int argc, char* argv[])
{
    ...
    std::cout << "GpsPosition implementation_level = " << implementation_level<GpsPosition>::value << '\n'
              << "GpsPosition tracking_level = " << tracking_level<GpsPosition>::value << '\n';

    std::cout << "\ntext_archive\n";
    {
        const GpsPosition gps(1, 2, 3.0f);
        saveLoad<text_oarchive, text_iarchive>("tar", gps);
    }
    std::cout << "\nbinary_archive\n";
    {
        const GpsPosition gps(1, 2, 3.0f);
        saveLoad<binary_oarchive, binary_iarchive>("bar", gps);
    }
    // std::cout << "\nsimple_log_archive\n";
    // {
    //     const GpsPosition gps(1, 2, 3.0f);
